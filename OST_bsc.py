...
def compress_DNA(original_file, final_huffmans_trees, window_length, start_time):
    ...
    for tree_key in temp:        
       	os.system("bsc e " + tree_key + ".txt " + tree_key + ".txt.bsc -t -T &>> " + original_file_name + ".log")
        print "window_length", window_length, "label_length", label_length, tree_key, os.path.getsize(tree_key + ".txt"), os.path.getsize(tree_key + ".txt.bsc"), float(os.path.getsize(tree_key + ".txt.bsc"))/os.path.getsize(tree_key + ".txt")
       	os.system("rm " + tree_key + ".txt")
        os.system("7za a " + original_file_name + ".ost.7z " + tree_key + ".txt.bsc &>> " + original_file_name + ".log")
      	s += os.path.getsize(tree_key + ".txt.bsc")
        os.system("rm " + tree_key + ".txt.bsc")
...
############ Decompression ###########################      
    ...
    # start the decompression process   # decompress the bins
    for key in tree_code_dictionary:
        os.system("bsc d " + tree_code_dictionary[key] + ".txt.bsc "  + tree_code_dictionary[key] + ".txt -t -T &>> " + labels_stream_bin_file + ".log")      
        os.system("rm " + tree_code_dictionary[key] + ".txt.bsc" + " &>> " + labels_stream_bin_file + ".log")
